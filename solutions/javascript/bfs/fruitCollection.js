...
For example, [2,4,5,10] indicates that we have a tree at the position (2,4) in a 2D grid, 
this tree has 5 fruits, and the monkey can jump to any tree within a 10 unit radius of this tree. 
Note that the monkey will never jump to another tree if its not possible to jump from the other tree to his current tree (since he doesn't want to get stranded).
...
const getWeightedComponentSize = (startNode, graph, visited, nodes) => {
    ...
    while (queue.size() > 0) {
        ...
        weightedComponentSize += currentNodeSize;
