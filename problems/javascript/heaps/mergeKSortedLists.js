 ...
 * @return {ListNode}
 */
const comparingNodes = (nodeOne, nodeTwo) => nodeOne.val - nodeTwo.val;

const mergeKLists = (lists) => {
    const sentinel = new ListNode();
    let tailOfMergedList = sentinel;

    const smallestRemainingNodePerArray = new MinHeap([], comparingNodes);

    for (const list of lists) {
        if (list !== null) smallestRemainingNodePerArray.push(list);
    }
...
console.log(`Your answer: ${mergeKLists(lists)}`);
console.log(`Correct answer: ${[1, 1, 2, 3, 4, 4, 5, 6]}`);
console.log();

lists = [];

console.log(`Your answer: ${mergeKLists(nums, k)}`);
