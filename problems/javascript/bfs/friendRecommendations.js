...
const getChildren = (node, matrix) => {
    const neighbors = [];
    const matrixNodeLength = matrix[node].length;

    for (let childNode = 0; childNode < matrixNodeLength; childNode++) {
        if (matrix[node][childNode] === FRIEND && childNode !== node) neighbors.push(childNode);
    }
