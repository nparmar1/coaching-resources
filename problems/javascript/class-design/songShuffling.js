...
and you expose 2 functions `addSong` and `playSong` which the correct inputs and outputs, 
...
const getRandomInteger = (startRange, endRange) => {
    const range = endRange - startRange;
    const randomInteger = startRange + Math.floor(Math.random() * range);
...
class Shuffle {
    ...
    playSong() {
        if (this.songs.length === 0) {
            console.error('Error: No songs have been added. Please add some songs');
            return;
        }

        if (this.songIdxOfUnplayedSongs >= this.songs.length) this.restartSongList();

        const randomSongIdx = getRandomInteger(this.songIdxOfUnplayedSongs, this.songs.length);

        const currentSong = this.songs[randomSongIdx];

        this.swapSongs(this.songIdxOfUnplayedSongs, randomSongIdx);
        this.songIdxOfUnplayedSongs++;
